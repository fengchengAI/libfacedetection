...
SConvInfo param_pConvInfo[NUM_CONV_LAYER] = {
    ...
    //conv6_3
    { 1, 1, 3, 3,128,128 },
    //loc3  这里的12是３＊４，三个anchor，４纬度坐标
    { 1, 1, 3, 3, 64, 12 },
    //conf3　 同理这里的６是３＊２，三个anchor，２纬度置信度
    { 1, 1, 3, 3, 64,  6 },
    //loc4
    { 1, 1, 3, 3, 128,  8 },
    //conf4
...
/***************CONV6*********************/
    ...
    float pSizes3[3] = {10, 16, 24};  //min_size 可以查看model/caffe下的参数
    ...
    unsigned char * rgb_image_data, int width, int height, int step) //input image, it must be RGB (three-channel) image!
{
    ...
    {
        //short * p = ((short*)(result_buffer + 4)) + 6 * i; //为什么＋４? 加4是因为第一个数用来保存。为什么加１４２,可以用５
        short * p = ((short*)(result_buffer + 4)) + 5 * i; //为什么＋４? 加4是因为第一个数用来保存。为什么加１４２,可以用5
